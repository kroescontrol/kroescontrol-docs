...
start_server() {
    ...
    for i in {1..20}; do
        # Nextra doesn't have health endpoint, check if port is responding
        if curl -s http://localhost:$PORT >/dev/null 2>&1; then
            echo -e "\n${GREEN}Server started successfully!${NC}"
            echo -e "URL: ${BLUE}http://localhost:$PORT${NC}"
            echo -e "Documentation site: Nextra/Next.js"
...
show_status() {
    if lsof -i :$PORT >/dev/null 2>&1; then
        ...
        echo "  - Public docs: /public"
        echo "  - Internal docs: /internal (requires auth)"
